...
## Managed Trial の取得手順（お客様導線）

**取得ページと管理画面は分離**（デモ量産前提）。

| 役割 | URL |
|---|---|
| お客様向け取得 | Studio `/trial`（`VITE_TRIAL_PORTAL_URL`） |
| 管理者（一覧・失効） | Studio `/admin/trial` |

1. AI-Demo-Studio に同一の Upstash + `OPENAI_API_KEY` を設定
2. product_flow `.env.local` に `VITE_TRIAL_PORTAL_URL=http://localhost:3000/trial`（本番は `https://demo.axeon.jp/trial`）
3. `/ai` →「体験コードを取得」→ Studio `/trial?demo=iso-chat&return=...`
4. 発行されたコードを「体験コードで試す」に入力
5. 質問（残回数表示あり）

管理者のみ一覧・失効が必要な場合は `/admin/trial` + `TRIAL_ADMIN_SECRET`。
...
VITE_TRIAL_PORTAL_URL=http://localhost:3000/trial
```
